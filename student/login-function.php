...
{	
	...
	public function changePass($exam_roll,$password)
	{
		try
		{
			global $conn;
			...
			$query = "UPDATE `user_student` SET 
			`password` = '$new_password' WHERE `exam_roll` =".$exam_roll;
				if(!mysqli_query($conn, $query)){
				throw new Exception(  mysqli_error($conn) );
				}
	...
	public function doLogin($uname,$upass)
	{
		try
		{
			$stmt = $this->conn->prepare("SELECT * FROM user_student WHERE exam_roll=:uname");
			...
			if($stmt->rowCount() == 1)
			{
				...
				{
					$_SESSION['user_session'] = $userRow['exam_roll'];
					return true;
				}
				else
				{
